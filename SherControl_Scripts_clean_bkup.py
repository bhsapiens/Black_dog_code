...
def MidControl(X0, q, dq, F, C, Acc, PHI, dt, Gb, Gl, omega, vs, vf, theta, Phi, w, Phihk, qc, Cq, T):
    ...
    X_data_2 = np.loadtxt('X_data_2.txt')
    Y_data_2 = np.loadtxt('Y_data_2.txt')
    ...
    for i in range(0, 4):
        ...
        if F[i] < 0.1 * F_threshold:
            xv = 2 * Regression_GP(Kt, Cq, X_data_2)
            yv = mul * Regression_GP(Kt, Cq, Y_data_2)

        else:
            xv = 1 * Regression_GP(Kt, Cq, X_data_2)
            yv = 0 #1 * Regression_GP(Kt, Cq, Y_data_2)

        rv = np.array([1.0 * xv, yv, 0])
        Rv = np.dot(R, rv)
        ...
        fv[:, i] = 1.5 * np.array([abs(vf) * Rv[0], 1 * abs(vf) * Rv[1], 0])
    ...
    for i in range(0, 4):
        ...
        if F[i] < -0.01 * F_threshold:
            Epos[:, i] = np.zeros([3])  # Posture control signal to the ith leg
        else:
    ...
    dzai_a_h = np.zeros([4, 1])
    dzai_a_k = np.zeros([4, 1])
    kah = 0.5
    for i in range(0, 4):
        if F[i] < 0.01 * F_threshold:
            dzai_a_h[i] = kah * (- pitch_b - q[1, i])

    dzai_a_a = np.zeros([4, 1])
    kaa = 0.5
    for i in range(0, 4):
        if F[i] < 0.1 * F_threshold:
            dzai_a_a[i] = kaa * (roll_b - q[0, i])

    # ---------------- End ---------------------------------

    # ================ Lateral Extention reflex =========================
    zai_LER = np.zeros([4, 1])
    dzai_LER = np.zeros([4, 1])
    Zai_ler = np.zeros([4, 1])
    for i in range(0, 4):
        if abs(Acc[2]) > 0.1 * F_threshold:
            zai_LER[i] = 0.05 * np.sign(Acc[2]) * (abs(Acc[2]) - 0.1 * F_threshold)
        else:
            dzai_LER[i] = - 5 * (zai_LER[i] + zai_LER0[i])
            zai_LER[i] = dzai_LER[i] * dt + zai_LER0[i]
        if F[i] < 0.1 * F_threshold:
            Zai_ler[i] = zai_LER[i]
    ...
    zai_LRR_h = zai_LRR_h0
    zai_LRR_k = zai_LRR_k0
    dzai_LRR_h = np.zeros([4, 1])
    dzai_LRR_k = np.zeros([4, 1])
    Zai_LRR_h = np.zeros([4, 1])
    Zai_LRR_k = np.zeros([4, 1])
    Ts = Ts0
    dTs = np.zeros([4, 1])

    for i in range(0, 4):
        if i < 2:
            vlrr = np.array([-0.25, -1, 0])
        else:
            vlrr = np.array([-0.5, -2, 0])
        vLrr = vlrr  # np.dot(R, vlrr)

        if C[i] > 0.5:
            J = Jacobian_calculation(q, Gl, i)  # Jacobian for ith leg from CM
            ...
            Jn = np.dot(U, np.dot(S, V))
            Ji = inv(Jn)

            OMEGA = np.dot(Ji, vLrr)
            zai_LRR_h[i] = 0.25 * OMEGA[1]
            zai_LRR_k[i] = 0.5 * OMEGA[2]
            Ts[i] = 10
        else:
            if 5 < Ts[i] <= 6:
                if i < 2:
            ...
            dzai_LRR_h[i] = - 5 * zai_LRR_h[i]
            dzai_LRR_k[i] = - 5 * zai_LRR_h[i]

            zai_LRR_h[i] = dzai_LRR_h[i] * dt + zai_LRR_h0[i]
            zai_LRR_k[i] = dzai_LRR_k[i] * dt + zai_LRR_k0[i]

        dTs[i] = -5 * Ts[i]
        Ts[i] = dTs[i] * dt + Ts0[i]

        if F[i] < 0.1 * F_threshold:
            Zai_LRR_h[i] = zai_LRR_h[i]
            Zai_LRR_k[i] = zai_LRR_k[i]
    # -------------------------- End --------------------------------

    # % ------------------------Complete reflex---------------------------------
    rfxa = 1 * dzai_a_a + 1 * Zai_ler  # size: 4x1
    rfxh = 0 * dzai_a_h + 0 * Zai_LRR_h  # size: 4x1
    rfxk = 0 * dzai_a_k + 0 * Zai_LRR_k  # size: 4x1
    ...
    for i in range(0, 4):
        ...
        parameters[i + 16, 0] = rfxa[i]  # size: 1x1
        parameters[i + 16, 1] = rfxh[i]  # size: 1x1
        parameters[i + 16, 2] = rfxk[i]  # size: 1x1
